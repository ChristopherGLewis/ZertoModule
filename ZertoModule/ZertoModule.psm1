...
class VMFailoverIPAddress {
    [string] $VMName;
    [FailoverIPAddress[]] $FailoverIPAddresses

    #region base CTOR
    VMFailoverIPAddress ([string] $VMName) {
        $this.VMName = $VMName;
    ...
    VMFailoverIPAddress ([string] $VMName, [FailoverIPAddress[]] $FailoverIPAddresses ) {
        $this.VMName = $VMName;
        $this.FailoverIPAddresses = @();
        $FailoverIPAddresses | ForEach-Object {
            $this.FailoverIPAddresses += $_
        }
    }
    #endregion

    AddIPAddress ([FailoverIPAddress] $FailoverIPAddress) {
        $this.FailoverIPAddresses += $FailoverIPAddress
    }
}

# .ExternalHelp ZertoModule.psm1-help.xml
Function New-ZertoVMFailoverIPAddress {
    param (
    ...
    if ($FailoverIPAddress)  {
        [VMFailoverIPAddress] $NewZertoVMIP = [VMFailoverIPAddress]::New($VMName, $FailoverIPAddress);
    } else {
        [VMFailoverIPAddress] $NewZertoVMIP = [VMFailoverIPAddress]::New($VMName);
    }
...
Function Remove-ZertoAuthToken  {
    ...
    #This has already been done in Get-ZertoAuthToken
    #Set-SSLCertByPass
...
Function Get-ZertoPeerSiteID {
    ...
    $Result = Get-ZertoPeerSites -ZertoServer $ZertoServer -ZertoPort $ZertoPort -ZertoToken $ZertoToken | `
                        Where-Object {$_.PeerSiteName -eq $ZertoSiteName} | `
                        Select-Object SiteIdentifier -ExpandProperty SiteIdentifier
    return $Result
}
#endregion
...
Function Add-ZertoVPG {
    [cmdletbinding(SupportsShouldProcess=$True)]
     param(
        ...
        [Parameter(Mandatory=$true, ParameterSetName="VM Names",  HelpMessage = 'Zerto Virtual Machine names')] [string[]] $VmNames,
        [Parameter(Mandatory=$true, ParameterSetName="VM Names and IP Addresses", HelpMessage = 'Zerto Virtual Machine names and failover IP addresses')] [VMFailoverIPAddress[]] $VmNamesAndIPAddresses
    ...
    $NewVPGJson = $NewVPGHash | ConvertTo-Json -Depth 10
...
# .ExternalHelp ZertoModule.psm1-help.xml
Function Stop-ZertoVPGFailover {
    param(
        [Parameter(Mandatory=$false, HelpMessage = 'Zerto Server')] [string] $ZertoServer = "il1zerto.nuveen.com",
        [Parameter(Mandatory=$false, HelpMessage = 'Zerto Server URL Port')] [string] $ZertoPort = 9669,
        [Parameter(Mandatory=$true, ValueFromPipeline=$true, HelpMessage = 'Zerto authentication token from Get-ZertoAuthToken')] [Hashtable] $ZertoToken,
